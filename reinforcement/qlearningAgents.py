...
class QLearningAgent(ReinforcementAgent):
    ...
    def getAction(self, state):
        ...
        rn = random.random()
        if rn < self.epsilon:
            rd = math.floor(random.random() * len(legalActions))
            action = legalActions[rd]
        else:
    ...
    def update(self, state, action, nextState, reward):
        ...
        value = (1 - self.alpha) * self.getQValue(state, action) + self.alpha * sample1
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        f = self.featExtractor.getFeatures(state, action)
        for key in f.keys():
            self.weights[key] += self.alpha * diff * f[key]
        #util.raiseNotDefined()
