        ...
        while not converge:
            ...
            for x, y in dataset.iterate_once(batch_size):
                if self.get_prediction(x) != nn.as_scalar(y):
                    self.get_weights().update(nn.Constant(x.data), nn.as_scalar(y))
                    converge = False
        pass
...
class DigitClassificationModel(object):
    ...
    def train(self, dataset):
        ...
        for x, y in dataset.iterate_forever(batch_size):
            if i == 49:
                if j == 0 and dataset.get_validation_accuracy() >= 0.96:
                    j += 1
                    self.lr /= 2
...
class LanguageIDModel(object):
    ...
    def __init__(self):
        ...
        self.w_hid = nn.Parameter(400, 400)
        self.b_hid = nn.Parameter(1, 400)
        self.w_final = nn.Parameter(400, len(self.languages))
        self.b_final = nn.Parameter(1, len(self.languages))
        self.params = [self.w, self.b, self.w_hid, self.b_hid, self.w_final, self.b_final]
    ...
    def run(self, xs):
        ...
        for i in range(len(xs)):
            if i == 0:
                layer_hid = nn.ReLU(nn.AddBias(nn.Linear(xs[i], self.w), self.b))
            else:
                layer_hid = nn.ReLU(nn.Add(nn.AddBias(nn.Linear(xs[i], self.w), self.b), nn.AddBias(nn.Linear(layer_hid, self.w_hid), self.b_hid)))
        output = nn.AddBias(nn.Linear(layer_hid, self.w_final), self.b_final)
        return output
    ...
    def train(self, dataset):
        ...
        for x, y in dataset.iterate_forever(batch_size):
            if i == 49:
                if j == 0 and dataset.get_validation_accuracy() >= 0.80:
                    j += 1
                    self.lr /= 2
